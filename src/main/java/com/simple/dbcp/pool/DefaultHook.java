...
public abstract class DefaultHook {
	...
	private DefaultHook(SimpleConfig config) {
		this.config = config;
	}
	...
	public static final class InitConnection extends DefaultHook implements Hook.InitConnection {
		public InitConnection(SimpleConfig config) {
			super(config);
		}
		...
		@Override
		boolean isEnabled() {
			return config.getInitSQL() != null || config.getDefaultAutoCommit() != null
					|| config.getDefaultReadOnly() != null || config.getDefaultTransactionIsolationIntValue() != null
					|| config.getDefaultCatalog() != null;
		}
	}

	public static final class GetConnectionTiming extends DefaultHook implements Hook.GetConnection {
		public GetConnectionTiming(SimpleConfig config) {
			super(config);
		}
	...
	public static final class CloseConnection extends DefaultHook implements Hook.CloseConnection {
		public CloseConnection(SimpleConfig config) {
			super(config);
		}
	...
	public static final class GetConnectionTimeout extends DefaultHook implements Hook.GetConnectionTimeout {
		public GetConnectionTimeout(SimpleConfig config) {
			super(config);
		}
	...
	public static final class QueryTiming extends DefaultHook implements Hook.StatementExecution {
		public QueryTiming(SimpleConfig config) {
			super(config);
		}
		...
		private void logQueryExecution(String sqlQuery, List<Object[]> sqlQueryParams, long takenNanos,
	...
	public static final class ResultSetSize extends DefaultHook implements Hook.ResultSetRetrieval {
		public ResultSetSize(SimpleConfig config) {
			super(config);
		}
